...
// New Functions :
//  1. DONE [ getOwnedTerit / setOwnedTerit] : which returns/sets a vector of owned territories (PLAYER)
//  2. DONE [ getReinforce / setReinforce]:  which returns avaliable reinforcement troops ( PLAYER ) and (GAMEENGINE)
//  3.  DONE [getUnattackable / setUnattackable] : which returns/sets a vector of unattackable players (PLAYER) 
//  4.  DONE [ getPlayerId] : return player id (PLAYER)
//  5.  NOT DONE because ownership is already expressed in the player's territory list [ setOwnerId] : sets owner of terittory id (MAP/Territory)
...
void Advance::execute() {
	if (validate()) {
		...
		if (targetAddress == nullptr && p != p2) {
			...
			int attackersKilled = round(static_cast<double>(target->getSoliders() * 0.7));
